...
public class Ninth1 extends JFrame implements ActionListener{
    ...
    public Ninth1() {
        ...
        l2 = new JTextArea("\n\n    "+ Second.name.getText() +" : (출튀는 무슨 그냥 열심히 듣자 정신차려!!)");
		l2.setOpaque(true);
    ...
    public void actionPerformed(ActionEvent e) { 
    	if (e.getSource() == b2) {
    		...
    		if (count == 1) {
    			ni.start();
    			l2.setText("\n    열정적으로 필기를 하고, 잠들지 않으려고 노력했다. 교수님의 질문세례를 받았다. \n    피곤함이 배가 되었다..");
    		}else if (count == 2) {
    			ni2.start();
    			l2.setText("\n    교수님 : "+Second.name.getText()+"학생 내일 오후에 저번에 제출한 과제 피드백 \n            받으러 오세요.");
    		}else if (count == 3) {
    			ni3.start();
    			l2.setText("\n\n    "+Second.name.getText()+" : 넵!(휴 다행이다 출튀했으면 못 들을 뻔 했네...)");
    		}else {
    			ni4.start();
